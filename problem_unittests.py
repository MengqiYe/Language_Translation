...
def test_decoding_layer(decoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        #    'Wrong shape returned.  Found {}'.format(train_decoder_output.rnn_output.get_shape().as_list())
        #assert infer_logits_output.sample_id.get_shape().as_list() == [batch_size, None], \
        #     'Wrong shape returned.  Found {}'.format(infer_logits_output.sample_id.get_shape().as_list())
